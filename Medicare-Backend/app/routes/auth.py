...
def reset_password(data: ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    # ✅ Generate keys now
    private_pem, public_pem = generate_ecc_keypair()
    enc = encrypt_private_key_with_password(private_pem, data.new_password)

    user.password_hash = hash_password(data.new_password)
    user.first_login = False

    user.public_key = public_pem.decode()
    user.private_key_encrypted = enc["ciphertext_b64"]
    user.private_key_salt = enc["salt_b64"]
    user.private_key_nonce = enc["nonce_b64"]
    user.private_key_iterations = enc["kdf_iterations"]

    db.commit()

    return {"message": "Password and keys initialized successfully"}