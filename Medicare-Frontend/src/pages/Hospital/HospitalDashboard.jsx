...
const HospitalDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const statsRes = await api.get("/hospital/stats");

      setStats({
        users: statsRes.data.users || 0,
        records: statsRes.data.records || 0,
        emergencies: statsRes.data.emergencies || 0,
      });
    } catch (err) {
      ...
      >
        ...
        <div className="relative z-10 text-center mb-12">
          ...
          <p className="text-gray-600 mt-2 text-sm md:text-base">
            Manage doctors, patients, medical records, and emergency access securely.
          </p>
        </div>
        ...
        <div className="relative z-10 text-center mt-16 text-gray-500 text-sm">
          Powered by <span className="text-cyan-600 font-semibold">MediChain Hospital Portal</span> — enabling
          secure and governed healthcare data access.
        </div>
      </motion.div>
    </DashboardLayout>
  );
